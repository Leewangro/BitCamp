...
public class App_home_2 {
    ...
    static void onTeamUpdate() {
        if (option == null ){
            ...
            for (int i = 0; i < teamIndex ; i++ ) {
                ...
                if (teams[i].name.equals(option)){
                    // Team[] teams = new Team[1000];을 통해 Team의 배열은 teams. 으로 접근한다.
                    Team team = new Team();//team은 단지 사용자의 입력을 저장함.
    ...
    static void onTeamDelete() {
        if (option == null ){
            System.out.println("해당 팀의 팀이 없습니다.");
        } else {
            for (int i = 0; i < teamIndex; i++){
                if(teams[i] == null) continue;
                if (teams[i].name.equals(option)) {
