...
public class ClassroomListServlet extends HttpServlet {
    ...
    private static final long serialVersionUID = 6910001558813528776L;
    ...
    protected void doGet(
        ...
        out.println("<title>강좌 목록</title>");
        out.println("</head>");
        out.println("<body>");
        out.println("<h1>강좌 목록</h1>");
        ...
        try {
            ...
            for (Classroom classroom : list) {
                ...
                out.printf("    <td><a href='view?no=%d'>%s</a></td>\n",
                        classroom.getNo(),
                        classroom.getTitle());
                out.printf("<td>%s~%s", classroom.getStartDate(),classroom.getEndDate());
                out.printf("<td>%s</td>", classroom.getRoom());
                out.println("</tr>");
            }
            out.println("/table");
        } catch (Exception e) {
            out.println("<p>목록 가져오기 실패!");
