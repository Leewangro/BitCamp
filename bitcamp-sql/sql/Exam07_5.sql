...
/* join ~ on 의 문제점
   => 반드시 on 에서 지정한 컬럼의 값이 같을 경우에만 
        두 테이블의 데이터가 연결된다.
   => 만약 연결되지 않은 데이터가 없다면 연결되지 않고, 결과로 출력되지 않는다.
*/
/* 전체 강의 목록 */
...
/* 강의 테이블에서 강의명을 가져오고, 강의실 테이블에서 지점명과 강의실 이름을 가져오자. */
select
	l.lno,
	l.titl,
	r.rno,
	r.loc,
	r.name
from lect l join room r on l.rno=r.rno;
/* inner join의 문제는 위의 경우처럼
 * 강의실이 아직 지정되지 않은 강의의 경우 강의실 테이블의 데이터와 연결하지 못해
 * 결과로 출력되지 않는 문제가 있다.
 * => 반드시 테이블에 같은 값을 갖는 데이터가 존재해야한다.
 */

select lno,titl,rno, loc, name 
from lect l join room r on l.rno = r.rno;
