...
insert into test1(title,rdt) values ('ooo',now());
insert into test1(title,rdt) values ('kkk',now());
insert into test1(title,rdt) values ('rrr',now());
...
insert into test2(filepath,bno) values('c:/download/f.gif',5);
...
위와 같이 test2 테이블ㅔ FK 제약 조건을 건 다음에 데이터를 입력해보자!
```
