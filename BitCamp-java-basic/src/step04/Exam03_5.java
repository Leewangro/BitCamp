//논리 연산자 : 비트 연산
package step04;

public class Exam03_5 {
public static void main(String[] args) {
        
        int a = 0b0110_1100;
        int b = 0b0101_0101;

        //정수 값에 대해서는 &&와 ||를 사용할 수 없다.
        //System.out.println(a && b);
        //System.out.println(a || b);

        //그러나 &, | , ^는 사용할 수 있다.
        //->각 비트 단위로 연산을 수행한다.
        //->1은 true, 0은 false라고 간주하고 계산한다.
        // -> 출력 결과도 정수이다
        // 비트 단위로 연산한다.
        System.out.println(a & b);
        System.out.println(a | b);
        System.out.println(a ^ b);
        //비트연산에서 not은 !이 아니라 ~이다.
        System.out.println(~a);//
        //
        
    }
}
//정리
// - 도대체 비트 연산자는 어디에 쓰이는가?
// -이미지 및 영상 처리에 사용된다.
// -예) 마스킹, 오버레이 기법에 사용된다.
// -예) 색조 변경에 사용된다.
//
// 사진을 컴퓨터에 저장하려면 2진수화 시켜야 한다.
// 일단 사진을 점(pixel)으로 바꾼다.
// 각 픽셀은 빨,파,초 (빛의 삼원색)으로 표현한다.
// 즉,빨강,파랑,초록의 색, 세개를 조절하여 다양한 색상을 표현하는 것이다.
// 보통 천연색을 표현하기 위해 rgb 각 색상을 8비트로 표현한다.
// 빨강(0~255), 파랑(0~255), 초록(0~255)
// 이 3개의 값을 합쳐서 한 개의 픽셀 색상을 표현하는 것이다.
// 즉 24비트(3바이트)로 픽셀 색상을 표현하는 것이다.
// 빨강 색의 한 픽셀 : 1111 1111 0000 0000 0000 0000 -0xFF0000
// 초록 색의 한 픽셀 : 0000 0000 1111 1111 0000 0000 -0x00FF00
// 파랑 색의 한 픽셀 : 0000 0000 0000 0000 1111 1111 -0x0000FF
// 파랑 색의 한 픽셀 : 1111 1111 1111 1111 0000 0000 -0x0000FF
