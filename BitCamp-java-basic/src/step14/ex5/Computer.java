...
public interface Computer {
    ...
    // 위에 touch라는 새 규칙을 추가하더라도
    // 기존에 작성한 FirstComputer,SecondConputer,ThirdComputer 클래스 모두
    // 컴파일 오류가 발생하지 않는다.
    // 왜? 구현된 메서드이기 때문이다.
    //
    // 가능한 일반 클래스의 메서드처럼 사용하지 말아라!
    // 새 규칙을 추가하는 의미로 default 메서드를 사용해야지
    // 일반 클래스의 메서드처럼 상속 해줄 목적으로 default메서드를 만들어서는 안된다.
}
