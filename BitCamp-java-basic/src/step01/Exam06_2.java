...
public class Exam06_2{
    public static void main(String[] args){
       //부동소수점도 메모리 크기에 따라 작성하는 방법이 다르다.
       System.out.println(3.14);//8바이트 크기의 부동소수점
       System.out.println(3.14f);//4바이트 크기의 부동소수점 -> 주로 소문자를 붙임
    
       //궁금
       //->정수는 메모리의 크기에 따라 최댓값과 최소값의 크기가 정해진다
       //->부동소수점은 최대/최소값은?
       ...
       //표시 범위가 4바이트 메모리 범위를 넘어가기 때문에 값이 짤린다.
       ...
       //정확하게 2진수로 딱 떨어지지 않는 경우가 존재한다. 
       //cpu,os,컴파일러,jvm의 문제가 아니다.IEEE-754의 방법에 내제되어 있는 문제다
       //해결방법
       //시스템에서 필요한 만큼 소수점 이하 자리수를 적절히 잘라 사용하라.

       //결론 
       //32비트 메모리에 부동소수점을 저장할 때는 
       //유효 자릿수 7자리까지(대부분) 가능하다.
       //65비트 메모리에 부동소수점을 저장할 때는
       //유효 자릿수 15자리까지(대부분) 가능하다.
       //그래서 32비트 float을 단정도(single-precision)라고 부르고
       //64비트는 double 타입(64-bit double-precision; 배정도)    
...
//   음수는 1, 양수는 0.
// - 지수(exponent)
//   127 bias를 사용한다. 즉 2의 지수 값에 127을 더한 결과 값을 사용한다.(Excess-k값을 사용함)
