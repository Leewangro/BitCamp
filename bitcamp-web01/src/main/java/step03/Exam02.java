...
public class Exam02 extends GenericServlet {
    ...
    //값을 내보낼 때 버전 번호도 내보내야됨.
    private static final long serialVersionUID = 1L;
    
    @Override
    public void service(ServletRequest request, ServletResponse response) throws ServletException, IOException {
        
     // => 출력하기 전이 아니라 출력스트림(PrintWriter out = response.getWriter();)을 꺼내기 전에 써야함
        ...
        // 출력 스트림을 꺼낼 때
        // 문자코드를 출력할 때 사용할 문자코드표를 지정하지 않으면,
        // 기본으로 ISO-8859-1 문자코드코드표를 사용하여 문자코드열을 출력한다.
        // 즉, 다음과 같이 클라이언트로 문자코드열을 보낸다.
        // String 객체(UTF-16) ==> ISO-8859-1
        // 즉, String 객체에 들어있는 문자코드열을 ISO-8859-1 코드표에 따라
        // 변환하여 클라이언트로 보낸다.
...
// => 예)text/html, text/plain, application/excel 등....여러가지가 있다.
